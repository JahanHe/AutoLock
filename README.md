...
本项目由 [ts1/BLEUnlock](https://github.com/ts1/BLEUnlock) 1.12.2 改进而来，当前版本为 **1.13.5 中文版（构建 808）**。新增原生设置窗口、菜单栏红绿状态灯、分组开关、逐项解释和效果预览。应用、菜单、窗口、安装文件和诊断包统一使用 **AutoLock**，安装后为 `AutoLock.app`。为继承已有设置、钥匙串和登录项，内部应用标识继续使用原值。

> 1.13.5 修正顶部透明效果：窗口本身透明，原生毛玻璃直接混合窗口后方内容，下方设置渐变过渡到清晰背景。右侧以 Mac 屏幕示意实时显示亮屏、关屏、锁定和屏保，连接灯、信号环与 60 秒曲线同步更新；锁定请求必须等系统确认后才显示已锁定。断连倒计时及“本次不锁定”固定在右侧顶部。此前本地迭代的紧凑单页设置、菜单栏快捷开关、分段亮屏、附近保持亮屏、断连通知取消与 Dock 选项一并纳入本次发布。

[下载 Apple 芯片安装包](https://github.com/JahanHe/AutoLock/releases/download/1.13.5/AutoLock-1.13.5-arm64.dmg) · [发布记录与源码](https://github.com/JahanHe/AutoLock/releases/tag/1.13.5)
...
菜单栏可直接切换离开自动锁定、断连后自动锁定、靠近动作、靠近亮屏、附近保持亮屏和 Apple Watch 兼容模式；“更多设置”提供被动模式、断连通知、锁定后关屏、媒体、Dock、信号数值和登录启动。勾选状态与窗口设置同步，相关选项关闭时联动禁用。设备扫描、日志、效果测试和权限入口会直接展开对应设置，保留重新连接、本次断连不锁定、立即锁定、关于和退出。
...
远离后手动解锁会重新确认距离；如果仍在远处或持续失联，会再次按所选策略锁定；已点击“本次不锁定”的断连，在有效信号恢复前保持豁免。关闭自动锁定则不执行这些锁屏动作，窗口明确显示“自动锁定已关闭”。
...
分段亮屏在信号从门槛外重新进入、或失联后恢复信号时触发。持续停留在远离区间不会刚锁定就反复亮屏；在达到更近门槛之前，本应用保持密码输入关闭。
...
仅修改 Swift/C/Objective-C 代码时，可以用 `scripts/build_local.sh /绝对路径/资源模板.app /绝对路径/AutoLock.app [签名证书]` 在本机重编译主程序和登录启动器，默认使用临时签名。模板必须来自相同界面与图标源码的可信构建；脚本会复用编译后的 nib 和图标，修改这些资源后必须先用 Xcode 重编译，不能继续复用旧模板。本次界面资源沿用 1.13.1，SwiftUI 页面和所有代码按 1.13.5 重编译。包内 `Contents/Resources/源码构建信息.json` 记录源文件校验值与资源基线；运行记录和导出的诊断包包含本地源码校验值。本次源码与 README 已同步到本仓库，发布标签 `1.13.5` 对应相同源码；安装包内记录该提交与源码校验值。
...
当前本地版本通过 117 项行为与日志检查。自动检查覆盖断连宽限、通知取消使用的真实计时器、恢复保护、过期弱信号防误锁、保持亮屏的生效与释放条件，以及信号无效、阈值、失联、关闭蓝牙、重新解锁、Apple Watch 密码互斥，以及日志持久化、轮转、压缩导出、清空和错误返回。蓝牙广播、系统深度睡眠、私有系统接口和 Apple Watch 的实机表现仍要按验收步骤测试，不能以“编译通过”代替稳定性验证。
