...
> 1.13.6 将锁定后的关屏改为降低亮度：默认 5%，可调 1% 至 30%；靠近或手动解锁后恢复原亮度，保留锁屏保护。支持的屏幕在低亮度下防止空闲关屏，不支持亮度调节的外接屏保持原状并报告原因。修复设备不在附近时解锁后反复锁定：默认暂停自动锁定，继续扫描，直到所选设备再次提供有效信号；可在“离开锁定”中关闭这项行为。补齐未断连就返回、远离确认未结束就返回，以及熄屏后已有近处信号时的亮屏触发。亮屏请求会检查系统返回值、有限重试并确认屏幕状态，结果可在右侧和运行记录查看。透明顶部、紧凑单页设置与实时屏幕状态保留。
