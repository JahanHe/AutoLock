...
下面是应用在 macOS 27 上实际运行并截图的原生窗口，设备与信号使用隔离演示数据；不是设计稿，也不代表已经完成真实离开、唤醒或手表解锁验收。
...
自动检查覆盖信号无效、阈值、失联、关闭蓝牙、重新解锁、Apple Watch 密码互斥，以及日志持久化、轮转、压缩导出、清空和错误返回。蓝牙广播、系统深度睡眠、私有系统接口和 Apple Watch 的实机表现仍要按验收步骤测试，不能以“编译通过”代替稳定性验证。
