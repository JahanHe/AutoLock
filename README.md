...
离开时自动锁定 Mac；回来时可以恢复原亮度或唤醒屏幕，把解锁交给 Apple Watch、Touch ID 或自己输入密码。

本项目由 [ts1/BLEUnlock](https://github.com/ts1/BLEUnlock) 1.12.2 改进而来，当前版本为 **1.13.7 中文版（构建 810）**。新增原生设置窗口、菜单栏红绿状态灯、分组开关、逐项解释和效果预览。应用、菜单、窗口、安装文件和诊断包统一使用 **AutoLock**，安装后为 `AutoLock.app`。为继承已有设置、钥匙串和登录项，内部应用标识继续使用原值。

> 1.13.7 简化应用顶部：原生红黄绿按钮旁只显示 AutoLock，使用随系统深浅色变化的纯色背景，移除毛玻璃、渐变透明和蓝橙背景对照。保留紧凑单页设置、右侧实时状态，以及锁定后降低亮度、靠近恢复亮度和无设备解锁后等待设备等功能。

[下载 Apple 芯片安装包](https://github.com/JahanHe/AutoLock/releases/download/1.13.7/AutoLock-1.13.7-arm64.dmg) · [发布记录与源码](https://github.com/JahanHe/AutoLock/releases/tag/1.13.7)

## 实际运行窗口截图

下面是应用在 macOS 27 上实际运行的原生窗口合成截图。设备信号、锁定和关屏场景使用隔离演示数据，不操作真实系统，也不代表已经完成手机离开、唤醒或手表解锁验收。
...
- 重新靠近，信号达到 **-90 dBm**：默认先恢复原亮度，屏幕已被系统关闭时再尝试唤醒；达到 **-60 dBm** 才允许已主动开启的本应用密码解锁。两道门槛可以分别调整。
- **Apple Watch 兼容模式默认开启，应用输入密码默认关闭。** 升级到本版时也会先关闭旧版自动输入密码设置，可以在窗口里主动开启。
- 设备不在附近时手动解锁，默认暂停自动锁定，直到再次收到所选设备的有效信号；可关闭此选项。
- 默认锁定后降低屏幕亮度至 **5%**，不主动发送关闭显示器命令。新选项“锁定后降低亮度”可关闭，亮度可设为 1% 至 30%。

应用先请求系统锁定，等待系统确认后再降低亮度或启动屏保。屏保不再替代锁定，也不依赖“屏保开始后多久要求密码”的设置。
...
菜单栏可直接切换离开自动锁定、断连后自动锁定、靠近动作、靠近亮屏、附近保持亮屏和 Apple Watch 兼容模式；“更多设置”提供被动模式、断连通知、锁定后降低亮度、媒体、Dock、信号数值和登录启动。勾选状态与窗口设置同步，相关选项关闭时联动禁用。“更多设置”还可切换“手动解锁后等待设备”。设备扫描、日志、效果测试和权限入口会直接展开对应设置，保留重新连接、本次断连不锁定、立即锁定、关于和退出。

## 设置窗口的分组

取消最左侧分类，所有设置集中在一个可连续向下滚动的页面。顶部只在原生红黄绿按钮旁显示 AutoLock，与内容区共用不透明的系统背景色，随系统切换深浅色，保留拖动窗口、关闭、最小化和全屏。菜单快捷入口仍可跳到对应设置。卡片按内容紧凑排列，功能名称和开关左右对齐，没有固定高度留白。**收起时只显示必要名称、常用开关和当前状态，不显示说明小字、数值输入或详细操作；点击卡片标题或空白处展开后，才显示完整设置、设备扫描、参数调整、测试按钮和解释。** 分组内部仅用分割线，不嵌套卡片。设置即时保存，右侧用 Mac 屏幕示意、信号强弱环、最近 60 秒曲线和倒计时进度条展示实时状态，再配合设备、权限和条件文字；异常原因、断连倒计时及“本次不锁定”入口始终可见。“查看运行记录”会直接展开日志。效果模拟在“效果测试”展开后按需打开，不操作真实系统。

| 分组 | 内容 |
| --- | --- |
| 状态总览 | 当前信号和监测状态；展开后显示运行判断和操作反馈。 |
| 随身设备 | 扫描、选择设备、被动模式、列表最低信号强度。一次监测一台设备。 |
| 离开锁定 | 自动锁定开关、远离门槛、确认时间、断连通知、无信号确认时间、提醒后的锁定等待、“本次不锁定”、断连时不锁定选项、锁定后降低亮度及亮度百分比，或启动屏保。 |
...
| 效果测试 | 真实锁屏、调暗、恢复亮度、唤醒、屏保、媒体、通知，以及锁定后延时测试靠近动作。 |
...
“锁定后降低亮度”默认开启。系统确认锁定后，将支持的屏幕降到 5%，保留可见内容；可调整为 1% 至 30%，不允许设为零。已经比目标更暗的屏幕不会被调亮，也不会使用关闭显示器作为备用方案。低亮度期间会防止空闲关屏，但仍尊重主动睡眠和合盖。

靠近动作开启且返回条件满足、手动解锁、关闭调暗选项或退出应用时，恢复每块屏幕各自的原亮度。重复锁定不覆盖第一次保存的原值；恢复记录写入偏好，应用下次启动时可恢复遗留亮度。已断开的屏幕保留恢复记录，重新连接后可点“恢复原亮度”。如果调暗后用户或系统已更改亮度，保留新的选择。

当前使用系统 DisplayServices 接口，支持内置屏及部分 Apple 显示器；系统不支持调节的外接屏保持原亮度，不代替显示器做 DDC 控制，不用黑色遮罩模拟亮度。右侧和日志会逐屏显示成功、失败或不支持，界面把低亮度与真正的系统关屏分开。“效果测试”提供“调暗屏幕”和“恢复原亮度”。这是私有系统接口，未来 macOS 更新可能影响兼容性。

## 附近保持亮屏与断连通知

“附近保持亮屏”使用 macOS 原生空闲显示器休眠抑制，不会修改系统电源设置。收到 6 秒内的有效新信号，且原始值、平均值都达到附近门槛时保持屏幕亮着。附近门槛取亮屏与远离门槛中较近的一档，默认 -80 dBm，优先保证离开锁定。失联、离开、锁屏、主动关屏、系统休眠、关闭开关或退出应用时停止附近保持亮屏；右侧显示是否正在生效，失败原因写入日志。锁定后的低亮度待机另行保持屏幕可见，直到恢复亮度或主动休眠，不受“附近保持亮屏”开关影响。
...
4. **靠近唤醒：** 开启锁定后降低亮度，离开并锁定后再回来，观察是否恢复原亮度（系统已关屏时观察是否点亮）。
...
仅修改 Swift/C/Objective-C 代码时，可以用 `scripts/build_local.sh /绝对路径/资源模板.app /绝对路径/AutoLock.app [签名证书]` 在本机重编译主程序和登录启动器，默认使用临时签名。模板必须来自相同界面与图标源码的可信构建；脚本会复用编译后的 nib 和图标，修改这些资源后必须先用 Xcode 重编译，不能继续复用旧模板。本次界面资源沿用 1.13.1，SwiftUI 页面和所有代码按 1.13.7 重编译。包内 `Contents/Resources/源码构建信息.json` 记录源文件校验值与资源基线；运行记录和导出的诊断包包含本地源码校验值。本次源码与 README 已同步到本仓库，发布标签 `1.13.7` 对应相同源码；安装包内记录该提交与源码校验值。
...
演示模式使用独立临时偏好，不创建蓝牙管理器、不访问登录密码、不注册登录项，也不会执行锁屏、按键、通知、脚本或屏幕操作。可追加 `--capture /绝对路径/预览目录` 运行 73 项开关联动与隔离检查、1 项分组跳转检查，并导出真实窗口的浅色、深色、各分组及失联截图，随后自动退出，共 15 张截图。加上 `--capture-composited` 可使用 macOS 合成截图，仅截取本应用窗口；此截图检查需要系统允许执行截图，正常使用 AutoLock 不需要录屏权限。窗口按钮可用 `AutoLock.app/Contents/MacOS/AutoLock --preview --check-window` 单独运行 4 项最小化／收起、关闭和恢复检查。也可运行 `scripts/check_preview.sh /绝对路径/AutoLock.app /绝对路径/预览目录` 自动核对全部截图。
...
当前本地版本通过 150 项行为与日志检查，包括亮度保存与恢复、重复调暗、无效值拒绝、外接屏不支持、修改失败、尊重用户后续调节、重启后按屏幕稳定标识恢复，以及设备缺席解锁暂停、有效信号恢复、开关回退和三类亮屏漏触发回归。自动检查覆盖断连宽限、通知取消使用的真实计时器、恢复保护、过期弱信号防误锁、保持亮屏的生效与释放条件，以及信号无效、阈值、失联、关闭蓝牙、重新解锁、Apple Watch 密码互斥，以及日志持久化、轮转、压缩导出、清空和错误返回。蓝牙广播、系统深度睡眠、私有系统接口和 Apple Watch 的实机表现仍要按验收步骤测试，不能以“编译通过”代替稳定性验证。
